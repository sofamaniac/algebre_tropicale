...
def calcul_matrix(u):
    ...
    for i in range(n):
        ...
        u2 = u+u[:i]
        lp = u2.rfind(u[i])  # last_position(u2,u[i])
        # il faut initialiser la première ligne
        if i == 0:
            M[i, lp:n] = 1
        else:
            # il faut s'assurer, pour ne pas calculer le minimum d'une matrice vide,
            # que le sous mot que l'on regarde n'est pas un facteur de u
            if (max(0, lp-n)) <= i:
